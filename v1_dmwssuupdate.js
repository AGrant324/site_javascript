$(document).ready( function() { 
	...
	complexityissuecountweightinga = new Array(0,1,1.1,1.25,1.5,2,2.5,3,3.5,4,4.5,5,5.5);
	...
	mandfieldsremainingnamesa['dmwssu_referralorgtypeid'] = "Referral Org Type";
	...
	$('#dmwsvisit_status').change( function() {
		...
		$('#dmwsvisit_mandfieldsremaininglist').val(mandfieldsremainingnamesalerta);
		//alert($('#dmwsvisit_mandfieldsremaininglist').val());
		})
	...
	*/
	...
	$('#dmwsreferral_add_new').on('click', function(event) {
		...
		insertrefout = insertrefout + '</div>';
		insertrefout = insertrefout + '<div class="col-sm-2">';
		insertrefout = insertrefout + '<textarea id="dmwsreferral_orgname_'+newref+'" name="dmwsreferral_orgname_'+newref+'" class="mand mandcheck form-control" style="background-color: pink; color: black; text-align: left;" rows="1"></textarea>';
		insertrefout = insertrefout + '<input type=hidden id="dmwsreferral_gdprcompliant_'+newref+'" name="dmwsreferral_gdprcompliant_'+newref+'" value="No">';	
		insertrefout = insertrefout + '<div class="checkbox"><label><input type="checkbox" name="dmwsreferral_gdprcompliant_'+newref+'" value="Yes" />Confirmed GDPR Compliant</label></div>'
		insertrefout = insertrefout + '</div>';
	...
	function showhidesafegaurdingextras() {
		...
		if (safeguardingcheckboxstate == "checked"){
			if (($('#dmwssu_safeguarding').val() == "Red") || ($('#dmwssu_safeguarding').val() == "Amber") || ($('#dmwssu_safeguarding').val() == "Green")) {} else {
			...
			if (($('#dmwssu_dmwssafeguardeetypeid').val() == "Child")||($('#dmwssu_dmwssafeguardeetypeid').val() == "ServiceUser")||($('#dmwssu_dmwssafeguardeetypeid').val() == "SUCarerAdult")||($('#dmwssu_dmwssafeguardeetypeid').val() == "SUCarerChild")||($('#dmwssu_dmwssafeguardeetypeid').val() == "SUWorkChild")||($('#dmwssu_dmwssafeguardeetypeid').val() == "SUWorkVulnAdult")||($('#dmwssu_dmwssafeguardeetypeid').val() == "VulnerableAdultFam")) {} else {
	...
	function recalllisteners() {
		...
		$('.datepicker').change( function() {
		    ...
		    if ($(this).val() != null){
		        ...
		        var entereddatea = entereddate.split("/");
		        if (entereddatea.length == 3){
		    	        if ((entereddatea[0].length == 2)&&(entereddatea[1].length == 2)&&(entereddatea[2].length == 4)){
		    	            var date =entereddatea[0]+"/"+entereddatea[1]+"/"+entereddatea[2]; 
		    	        }
		    	        else{
		    	            if (entereddatea[0].length == 1){
		    	                entereddatea[0] = "0" + entereddatea[0]; 
		    	            }
		    	            if (entereddatea[1].length == 1){
		    	                entereddatea[1] = "0" + entereddatea[1]; 
		    	            }
		    	            if (entereddatea[2].length == 2){
		    	                var d = new Date();
		                        var n = d.getFullYear();
		                        var thisyear = n.toString();
		                        var yyyya = thisyear.split("");
		                        var yy = yyyya[2] + yyyya[3];
		                        if (entereddatea[2] == yy){
		                            entereddatea[2] = n;
		                        }
		                        else if (entereddatea[2] <= 30){
		                            entereddatea[2] = "20" + entereddatea[2];
		                        }
		                        else if (entereddatea[2] > 30){
		                            entereddatea[2] = "19" + entereddatea[2];
		                        }
		    	            }
		    	            var date =entereddatea[0]+"/"+entereddatea[1]+"/"+entereddatea[2];
		    	            if (entereddatea[0] < 1 || entereddatea[0] > 31){
		    	            		wrongday = "1";
		    	            }
		    	            if((entereddatea[1] < 1)||(entereddatea[1] > 12)){
		    	            		wrongmonth = "1";
		    	            }
		    	            if((entereddatea[2] < 1900)||(entereddatea[2] > 2025)){
			            		wrongyear = "1";
			            }
		    	            if((wrongday == "1")||(wrongmonth == "1")||(wrongyear == "1")){
		    	            	$.confirm({
		    						icon: 'fa fa-exclamation-circle text-danger',
		    					    title: 'Incorrect Date',
		    					    content: 'Please ensure dates are entered in a dd/mm/yyyy format.',
		    					    buttons: {
		    					    		somethingElse: {
		    					            text: 'OK',
		    					            btnClass: 'btn-orange',
	...
	*/
	...
	$('#dmwsaction_add_new').on('click', function(event) {
		...
		insertaction = insertaction + '<div class="col-sm-1"><input id="dmwsaction_date_'+newref+'" name="dmwsaction_date_'+newref+'_dd/mm/yyyy" class="datepicker form-control dmwsactiondate" value="">';
		...
		insertaction = insertaction + '<div class="col-sm-2"><input id="dmwsaction_completebydate_'+newref+'" name="dmwsaction_completebydate_'+newref+'_dd/mm/yyyy" class="datepicker form-control dmwsactiondate" value=""></div>';
		...
		if (dmwsactiontimebanda.length >0) {
			for (var dmwsactiontimebandi in dmwsactiontimebanda) {
				...
				Check_Data_Hash('dmwssu',dmwssu_id);
		...
		insertaction = insertaction + '</div>';
		insertaction = insertaction + '<div class="col-sm-2"><input id="dmwsaction_completiondate_'+newref+'" name="dmwsaction_completiondate_'+newref+'_dd/mm/yyyy" class="datepicker form-control dmwsactiondate" value=""></div>';
	...
	$('#dmwssafeguardingissuetype_add_new').on('click', function(event) {
		...
		var select = document.getElementById('disabledServiceType').cloneNode(true).innerHTML;
		var selectSplit = select.split('>');
		// console.log(selectSplit[1]);
		selectSplit[1] = '<select id="dmwsserviceprovided_dmwsservicetypeid_'+newref+'"  name="dmwsserviceprovided_dmwsservicetypeid_'+newref+'" class="form-control">';
		var selectOutput = "";
		for (var i = 0; i < selectSplit.length-1; i++) {
			selectSplit[i] += ">";
			selectOutput += selectSplit[i];
		}
		// selectSplit = selectSplit.replace(/,/g, ">");
		// console.log(selectOutput);
		// console.log(select);
		insertserviceprovided = insertserviceprovided + selectOutput;
		// console.log(insertserviceprovided);
		...
		//returnpoint
		// var dmwsservicetypea = Get_Array_Select('dmwsservicetype','','dmwsservicetype_disabled');
		// console.log(dmwsservicetypea);
		// $dmwsserviceprovideda = Get_Array_Select('dmwsserviceprovided','','dmwsservicetype_disabled');
	...
	$('#dmwscomplexity_add_new').on('click', function(event) {
		var currcomplexitycount = 0;
		$('.complexityslider').each(function () {
			currcomplexitycount++;
		});
		if ( currcomplexitycount < 12 ) {		
			var newref = TtimeStamp();
			var insertcomplex = "";
			insertcomplex = insertcomplex + '<div id="dmwscomplexity_issuediv_'+newref+'">';
    		...
    		$.alert({
    			...
    			title: "Warning",
    		    content: "Sorry: Maximum number of 12 Complexity Issues allowed."
    		});			
	...
	$('.wellbeingradio').on('click', function(event) {		
		var thisid = $(this).attr('id');
		var thisiconclass = $('#'+thisid+"_icon").attr('class');
		var thisiconcolor = $('#'+thisid+"_icon").css('color'); 
		var currentlychecked = "0";
		if (thisiconclass == "fa fa-check-square-o fa-2x") {			
			if (thisiconcolor == "rgb(153, 0, 153)") { currentlychecked = "1"; }
			if (thisiconcolor == "red") { currentlychecked = "1"; }
			if (thisiconcolor == "blue") { currentlychecked = "1"; }			
			if (thisiconcolor == "green") { currentlychecked = "1"; }						
		}		
...
//================ OUTCOMES & IMPACTS MANDATORY FIELDS SETUP ======================
	...
	if ((dmwsvisit_type == 'FollowUp')||(dmwsvisit_type == 'Discharge')){	
		$('#outcomesdiv1').css('background-color','pink');
	...
	$('a[data-toggle="tab"]').each(function(e) {		
        var currentTabTitle = $(this).html(); // get current tab
        if ( currentTabTitle.includes("*") )  { 
        	$(this).css("background-color","white");
	...
	*/
	...
	function handleDataRequestSuccess(data, status) {
	    // alert(data);
		if(data != undefined){
			// if ( JSPersonId() == "bbra" ) { $('#TRACETEXT').html(data); }
			Create_Hashes(data);
			...
			datepickerlistener();
			doblistener();
			admissiondatelistener();
			dischargedatelistener();			
			expecteddischargedatelistener();			
			// modspecificlistener();
	...
	function handleDataRequestFailure(xhr, reason, ex) {
		 alert("Error Reason DU001 = "+"|"+xhr+"|"+reason+"|"+ex+"|");
	...
	datarequestlist = datarequestlist + "dmwsconsentwithdrawaltype" + ",";	 	
 	...
 	datarequestlist = datarequestlist + "dmwsservicetype" + ",";
        if (dmwssu_id != "New") {
            datarequestlist = datarequestlist + "dmwssu[singlekey="+dmwssu_id+"]" + ",";
            datarequestlist = datarequestlist + "dmwssux[singlekey="+dmwssu_id+"]" + ",";
        }
...
function initialformatter() {
	...
	var todatetext = $('#dmwssu_expecteddischargedate').val();
	var fromdatetext = $('#dmwssu_dmwsadmissiondate').val();
	$('#dmwssu_losexpectedcalc').val(days_between(DDsMMsYYYYtoYYYY_MM_DD(fromdatetext),DDsMMsYYYYtoYYYY_MM_DD(todatetext)));
	var todatetext = $('#dmwssu_dischargedate').val();
	var fromdatetext = $('#dmwssu_dmwsadmissiondate').val();
	$('#dmwssu_losactualcalc').val(days_between(DDsMMsYYYYtoYYYY_MM_DD(fromdatetext),DDsMMsYYYYtoYYYY_MM_DD(todatetext)));			
...
function admissiondatelistener() { 	
	$('#dmwssu_dmwsadmissiondate').datepicker().on('changeDate', function(e){
	...
	$('#dmwssu_dmwsadmissiondate').change( function() {
		var fromdatetext = $(this).val();
		var todatetext = $('#dmwssu_expecteddischargedate').val();
		$('#dmwssu_losexpectedcalc').val(days_between(DDsMMsYYYYtoYYYY_MM_DD(fromdatetext),DDsMMsYYYYtoYYYY_MM_DD(todatetext)));
		var todatetext = $('#dmwssu_dischargedate').val();
...
function expecteddischargedatelistener() { 	
	$('#dmwssu_expecteddischargedate').datepicker().on('changeDate', function(e){
		var todatetext = $(this).val();
		var fromdatetext = $('#dmwssu_dmwsadmissiondate').val();
		$('#dmwssu_losexpectedcalc').val(days_between(DDsMMsYYYYtoYYYY_MM_DD(fromdatetext),DDsMMsYYYYtoYYYY_MM_DD(todatetext)));
	});	
	$('#dmwssu_expecteddischargedate').change( function() {
		var todatetext = $(this).val();
		var fromdatetext = $('#dmwssu_dmwsadmissiondate').val();
		$('#dmwssu_losexpectedcalc').val(days_between(DDsMMsYYYYtoYYYY_MM_DD(fromdatetext),DDsMMsYYYYtoYYYY_MM_DD(todatetext)));	
	});	
...
function dischargedatelistener() { 	
	$('#dmwssu_dischargedate').datepicker().on('changeDate', function(e){
		var todatetext = $(this).val();
		var fromdatetext = $('#dmwssu_dmwsadmissiondate').val();
		$('#dmwssu_losactualcalc').val(days_between(DDsMMsYYYYtoYYYY_MM_DD(fromdatetext),DDsMMsYYYYtoYYYY_MM_DD(todatetext)));
	});	
	$('#dmwssu_dischargedate').change( function() {
		var todatetext = $(this).val();
		var fromdatetext = $('#dmwssu_dmwsadmissiondate').val();
		$('#dmwssu_losactualcalc').val(days_between(DDsMMsYYYYtoYYYY_MM_DD(fromdatetext),DDsMMsYYYYtoYYYY_MM_DD(todatetext)));		
	});	
...
function countwellbeingscore() {
	...
	//alert("FINAL "+wellbeingScore);//
	 ...
	 $('#dmwswellbeing_score').val(wellbeingScore);
...
function loadcomplexityslider() {		
	Check_Data_Hash('dmwscomplexity',"Latest");
	if ( GLOBALS["IOWARNING"] == "0" ) {			
...
function dmwsactiondatelistener() {
	$('.dmwsactiondate').each(function() {			
		formChanged();
		$(this).datepicker({
		    startDate: '-3d',
		    format: 'dd/mm/yyyy'
