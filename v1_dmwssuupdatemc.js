$(document).ready( function() { 
	...
	$('.progdatabutton').on('click', function(event) {
		...
		if ( showprevprogdatatoggle == "0" ) { 
			...
			DMWSRadarChart.draw("#chart", sprevprogdataa, thisprogdataa, myradarchartcfg, "ShowPrevData");
		} else { 
			showprevprogdatatoggle = "0";
			 $(this).css('border-bottom-width',"1px");
			 $(this).css('border-bottom-color',ogbuttoncolour);
			 toggledprogbuttonsa[thisid] = "hide";
			DMWSRadarChart.draw("#chart", sprevprogdataa, thisprogdataa, myradarchartcfg, "HidePrevData");
	
		}
	})
	
function prevprogdatacheck() {
		
			$('.progdatabutton').each(function () {
			var thisid = $(this).attr('id');
			if ($(this).css('border-bottom-width') == "5px") {
				toggledprogbuttonsa.push(thisid);
			};
			var progressvisitida = Get_Array_Hash('dmwsprogress');
			for (var pi in toggledprogbuttonsa) {
				Get_Data_Hash('dmwsprogress',toggledprogbuttonsa[pi]);
				var da2 = [];
				var da3 = [];
				da3["axis"] = "Current Admission or Treatment";
				da3["value"] = GLOBALS['dmwsprogress_treatment'];
